...
class OrgSubscriptionPlan extends ConsumerStatefulWidget {
  final Map<String,dynamic> outputValue ;
  final String password;
  OrgSubscriptionPlan({required this.outputValue,required this.password});

  @override
  ConsumerState<OrgSubscriptionPlan> createState() => _SubscriptionPlanTestState();
}

class _SubscriptionPlanTestState extends ConsumerState<OrgSubscriptionPlan> {
  ...
  void initState(){
    super.initState();
    outputValue = widget.outputValue;
  ...
  Future<dartz.Either<String, dynamic>> subscriptionPlanOrg({
    ...
    try {
      final response = await dio.post(
          ...
          data: {
            "id": 0,
            "userid": '${outputValue['result']['orgId']}',
                    ...
                    child: ElevatedButton(
                        ...
                        onPressed: isPostingData
                          ...
                          if(schemePlan.schemeId! == 4){
                            await userRegisterOrganization().then((value) {
                              if(value.isLeft()){
  ...
  payWithKhaltiInApp({
    ...
    final config = PaymentConfig(
      ...
      productIdentity: outputValue['result']['orgId'],
